...
- Versioning for each service is resolved independently from the service's own `@versioned` definition. By default TCGC uses all versions up to the latest for each service. An emitter's `api-version` map can select a version per service; dotted service namespaces are represented as nested map entries.
