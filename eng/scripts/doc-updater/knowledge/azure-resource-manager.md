...
Rule doc files under `website/src/content/docs/docs/libraries/azure-resource-manager/rules/` are generated from each registered rule's `docs` file reference by `pnpm regen-docs`. The package script passes `--rules-dir ../rules`, relative to the `reference/` output directory. The `reference/linter.md` file is generated at the same time and links to these rule pages. The generated rule directory is ignored by Git because the website build regenerates it; durable rule content belongs in `packages/typespec-azure-resource-manager/src/rules/*.md`.

## Reference Documentation Generation

- `pnpm regen-docs` from `packages/typespec-azure-resource-manager/` generates tracked reference files and ignored rule pages under the sibling `rules/` directory.
- The package must be built before running `pnpm regen-docs`.

## Relationship Base Type (Experimental)

- `Relationship<Properties>` is an extension-resource template that applies the Relationship base
  type contract automatically.
- Its property bag must extend `RelationshipProperties<ProvisioningState>`, which supplies
  `baseTypes`, source and target resource and tenant identifiers, and `provisioningState`.
- Relationship operations use the `Azure.ResourceManager.Extension` operation templates because
  relationships can target arbitrary ARM scopes.
- Applying the base type in a provider namespace requires suppressing the `basetypes-experimental`
  warning.

## Billing Data Placement

`BillingDataProperty` is not an ARM envelope property. Spread it into the provider-specific resource
properties model, as shown by the `resource-common-properties/billing-data` sample.
...
- Child property bases `ConversationProperties` and `ResponseProperties` each require `input: InputItem`. Response mix-ins include `PreviousResponseProperty`, `ResponseOutputProperty`, and `ResponseInstructionsProperty`.
