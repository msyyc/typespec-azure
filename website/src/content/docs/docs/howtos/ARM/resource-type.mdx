...
| `Extension.ServiceGroup`    | Service group-level scope                 |

##### Note: Service Group Scope

Service groups are a grouping mechanism allowing you to create flexible, custom groupings of Azure resources. For more information on service groups, including when to use service groups instead of management groups, please see the official Azure documentation: [https://aka.ms/servicegroups](https://aka.ms/servicegroups).

If you previously modeled service group scope manually with `Extension.ExternalResource`, replace the custom alias with `Extension.ServiceGroup`:
...
// GET /providers/Microsoft.Management/serviceGroups/{serviceGroupName}/providers/Microsoft.ContosoProviderHub/employees/{employeeName}
@armResourceOperations
...
Use `Extension.ServiceGroup<"serviceGroupId">` if your API already uses a different path parameter name.

```typespec
// GET /providers/Microsoft.Management/serviceGroups/{serviceGroupId}/providers/Microsoft.ContosoProviderHub/employees/{employeeName}
@armResourceOperations
interface ServiceGroups extends EmplOps<Extension.ServiceGroup<"serviceGroupId">> {}
```
...
Billing Data provides a standard description of a resource's billing state, including a reference to
a product in the Marketplace Catalog. Resource providers with prepaid billing, where a customer is
charged before the product is provisioned or fulfilled, should use this pattern so customers can
interact with these resources through central experiences.

`BillingData` is available in ARM common types v6. Spread `BillingDataProperty` into the
resource-specific property bag, not the resource envelope:
