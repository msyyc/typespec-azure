...
### Billing Data for Prepaid Resources

Billing Data provides a standard description of a resource's billing state, including a reference
to a product in the Marketplace Catalog. It is strongly recommended for resource providers with
prepaid billing, where the customer is charged before the product is provisioned or fulfilled.

`BillingDataProperty` is available in ARM common-types v6 and later. Unlike standard ARM envelope
properties, spread it into the resource-specific properties model:
