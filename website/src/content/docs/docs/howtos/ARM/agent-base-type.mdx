...
- **Appliance** — the service owns and reports the agent configuration. Definition and agent
  properties are **read-only**, including the `definition` container itself.
...
`AgentDefinitionAppliance` or `AgentDefinitionPlatform`. Each template takes boolean value
parameters that control whether the optional definition properties are present:
...
For example, to include the optional properties in each deployment model:
...
`modelDeploymentRef` exists only in the Platform deployment model, where the client selects the
underlying model deployment. In the Appliance deployment model the service owns the entire agent
definition, so there is no `modelDeploymentRef` property and `AgentPropertiesAppliance.definition`
is read-only.
...
Both `ConversationProperties` and `ResponseProperties` require one `input` value of type
`InputItem`. The `type` discriminator determines which fields apply:

- `Message` uses `content`; its `role` is assigned by the service and is read-only.
- `FunctionCall` uses `callId`, `name`, and `arguments`.
- `FunctionCallOutput` uses `callId` and `output`.
- `Compaction` uses `summary`.

Message `content` and function `arguments` are `Record<unknown>` values rather than plain strings,
so they can carry structured input. Service-generated output uses the corresponding read-only
`ResponseItem` model.
