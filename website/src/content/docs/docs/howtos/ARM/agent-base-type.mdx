...
`ConversationProperties` and `ResponseProperties` each contribute the required `input: InputItem`
property. Clients must provide an input item when creating either child resource. Set its `type` to
the appropriate `ItemType` and populate the writable fields for that variant, such as `content` for
a message or `callId` and `output` for function-call output.
...
directly, apply the decorator to the resource model with a `BaseTypeInfo` value. It may be applied
multiple times to declare conformance to multiple base types; duplicate entries are ignored:

```typespec
model MyAgentProperties is AgentPropertiesPlatform<ContosoPlatformDefinition> {
  ...DefaultProvisioningStateProperty;
}
