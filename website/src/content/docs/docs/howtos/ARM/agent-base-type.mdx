...
`ConversationProperties` includes a required `input: InputItem` property. An `InputItem` uses its
`type` discriminator to represent a message, function call, function call output, or compaction
summary. Message `content` and function-call `arguments` are structured values modeled as
`Record<unknown>`.
...
on a resource model directly, apply the decorator with a `BaseTypeInfo` value. It may be applied
multiple times to declare conformance to multiple base types; duplicate entries are ignored:

```typespec
model MyDefinition is AgentDefinitionPlatform;
