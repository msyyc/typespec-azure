...
```typespec
@clientOption(name: valueof string, value: unknown | (valueof unknown), scope?: valueof string)
```
...
| --------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`    | `valueof string`               | The name of the option to set                                                                                                                      |
| `value`   | `unknown \| (valueof unknown)` | The option value. Primitive, array, object, nested, and model reference values are supported; the consuming emitter determines the expected shape. |
| `scope`   | `valueof string` (optional)    | The target language scope. Every use must provide an explicit scope; omitting it produces a `decorator-requires-scope` warning.                    |
...
The decorator supports arbitrary values, including primitives, arrays, objects, nested combinations, and model references:
...
For an option that needs a TypeSpec type, pass the model name as the value. TCGC converts the model reference to the applicable SDK type before the emitter reads the option. Customizations on the referenced model are preserved and resolved for the selected scope; for example, a C#-scoped model reference customized with `@alternateType(string, "csharp")` is exposed to the C# emitter as the SDK built-in `string` type.
