...
| `value`   | `unknown \| valueof unknown` | The option value. Literal values and TypeSpec model references are supported; the consuming emitter determines the expected shape. |
...
Model references can be used for structured options. The referenced model is resolved as an SDK model, preserving customizations such as `@alternateType` for the scoped emitter:
