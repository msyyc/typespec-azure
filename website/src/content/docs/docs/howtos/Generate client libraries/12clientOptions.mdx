...
| `value`   | `unknown \| valueof unknown` | The option value. Literal values and references to TypeSpec models are supported; the consuming emitter determines the expected shape. |
...
The decorator supports arbitrary literal values, including primitives, arrays, objects, and nested combinations:
...
An option value can reference a TypeSpec model instead of using a literal value. For example, `@clientOption("composes", ComposedOptions, "csharp")` makes `getClientOptions` return the corresponding `SdkModelType` to the C# emitter. TCGC resolves the model through the normal client type graph, so customizations on the referenced model are preserved. If `ComposedOptions` has `@alternateType(string, "csharp")`, the option value resolves to the alternate string type for the C# emitter.
