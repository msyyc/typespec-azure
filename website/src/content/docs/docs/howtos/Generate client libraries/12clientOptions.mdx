...
The decorator signature is `@clientOption(name: valueof string, value: unknown | valueof unknown, scope?: valueof string)`.
...
| `value`   | `unknown \| valueof unknown` | The option value. Literal primitives, arrays, objects, nested values, and TypeSpec model references are supported; the consuming emitter determines the expected shape. |
...
A client option can reference a TypeSpec model directly. For example, `@clientOption("composes", SharedOptions, "csharp")` makes `getClientOptions` return the corresponding `SdkModelType` to the C# emitter instead of reducing the reference to a plain object. TCGC applies scoped customizations while resolving that model: if `SharedOptions` has a matching `@alternateType`, the option value is the resolved alternate SDK type.
