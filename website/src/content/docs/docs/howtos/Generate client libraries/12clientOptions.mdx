...
A TypeSpec model reference can also be used as the option value. The referenced
model is resolved by the scoped emitter, including any applicable model
customizations.

```typespec title=client.tsp
model CreateResponseOptions {
  mode: string;
}

#suppress "@azure-tools/typespec-client-generator-core/client-option"
@clientOption("composes", CreateResponseOptions, "csharp")
