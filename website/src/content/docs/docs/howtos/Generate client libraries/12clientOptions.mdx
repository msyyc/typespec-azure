...
| `value`   | `unknown \| valueof unknown` | A literal or structured value, or a reference to a TypeSpec model. Model references are resolved to the corresponding SDK type, including applicable customizations such as `@alternateType`. |
...
The decorator supports arbitrary values, including primitives, arrays, objects, nested combinations, and references to TypeSpec models:
...
For a model-valued option, use the model name directly, such as `@clientOption("composes", ComposedOptions, "csharp")`. `getClientOptions` returns the model's resolved SDK type rather than its TypeSpec declaration. This preserves scoped customizations: if `ComposedOptions` has `@alternateType(string, "csharp")`, the C# emitter receives the SDK built-in `string` type for the option value.
