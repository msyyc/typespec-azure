...
When your spec defines multiple services merged into a single client (using `@client({service: [ServiceA, ServiceB]})`), you can specify a different API version for each service by passing an object instead of a string. The keys identify service namespaces, and the values are the desired version strings.

Services not listed in the object default to their latest version.

For top-level service namespaces, map each namespace directly:
...
For nested service namespaces such as `Microsoft.Network` and `Microsoft.Compute`, represent each namespace segment as a nested object in `tspconfig.yaml`:
...
This nested form resolves `Microsoft.Network` to `2024-01-01` and `Microsoft.Compute` to `2024-05-01`. Do not write dotted namespace names as YAML keys.
