...
When your spec defines multiple services merged into a single client (using `@client({service: [ServiceA, ServiceB]})`), you can specify a different API version for each service by passing an object instead of a string. Each value is the desired version string. Top-level service namespaces can be direct keys:
...
For dotted service namespaces, represent each namespace segment as a nested object in `tspconfig.yaml`. For example, configure `Microsoft.Network` and `Microsoft.Compute` as follows:
...
TCGC resolves these entries to the full service namespace names `Microsoft.Network` and `Microsoft.Compute`. The selected version controls both the generated API surface and which versioned examples TCGC loads for each service.
