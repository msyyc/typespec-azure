...
When your spec defines multiple services merged into a single client (using `@client({ service: [Contoso.ServiceA, Contoso.ServiceB] })`), you can specify a different API version for each service by passing an object instead of a string. In `tspconfig.yaml`, represent each service namespace as nested keys, one key per namespace segment, and set the desired version at the leaf.
...
With this configuration, the generated client uses version `2024-01-01` for `Contoso.ServiceA` operations and `2024-05-01` for `Contoso.ServiceB` operations. Programmatic emitter options can instead use flat fully qualified keys, such as `{ "Contoso.ServiceA": "2024-01-01" }`.

If `Contoso.ServiceB` is omitted, it defaults to its latest version, `2024-09-01`.
