...
When your spec defines multiple services merged into a single client (using `@client({ service: [Microsoft.Network, Microsoft.Compute] })`), you can specify a different API version for each service by passing an object instead of a string. Each namespace segment is a key in a nested object, and the leaf value is the desired version string. A one-segment namespace such as `ServiceA` is therefore a direct key, while `Microsoft.Network` is configured as `Microsoft: { Network: "2024-01-01" }`.
...
For example, given two versioned services in nested namespaces:
...
With the configuration above, the generated client will use version `2024-01-01` for `Microsoft.Network` operations and `2024-05-01` for `Microsoft.Compute` operations.

If `Compute` is omitted from the nested object, `Microsoft.Network` will use `2024-01-01` while `Microsoft.Compute` defaults to its latest version, `2024-09-01`.
...
```csharp
// The C# emitter generates both operations and a ServiceVersion enum, but does not
// emit version guards on the newly added operation.
namespace My.Service
{
  ...
  public partial class ServiceClient
  {
    ...
    public virtual Response GetUserInfo(UserInfo userInfo, CancellationToken cancellationToken = default);
    public virtual Task<Response> GetUserInfoAsync(UserInfo userInfo, CancellationToken cancellationToken = default);
    public virtual Response CreateUserProfile(UserProfile userProfile, CancellationToken cancellationToken = default);
    public virtual Task<Response> CreateUserProfileAsync(UserProfile userProfile, CancellationToken cancellationToken = default);
  }

  public partial class ServiceClientOptions : ClientOptions
  {
    public ServiceClientOptions(ServiceVersion version = ServiceVersion.V2024_02_01);
...
```csharp
// The C# emitter generates the new model and property as plain members; it does not
// emit version guards or per-version conditional shapes.
namespace My.Service
{
  public partial class UserInfo
  {
  ...
  public partial class UserProfile
  {
    public UserProfile(string profileName, ProfileDetails profileDetails);
    public string ProfileName { get; }
    public ProfileDetails ProfileDetails { get; }
  }
...
### Keeping an Api Version Parameter on the Operation with `@apiVersion(false)`

In some cases, a service may have an `api-version` query parameter on an operation that should **not** be treated as the client-level API version parameter. For example, when the parameter controls a downstream service version or has a different semantic meaning for a specific operation. You can use `@apiVersion(false)` to explicitly opt a parameter out of API version treatment.

When `@apiVersion(false)` is applied to a parameter:

- That parameter will **not** be treated as an API version parameter
- It stays as a method-level parameter and is **not** elevated to the client
- Other operations in the same client can still have their API version parameters elevated to the client normally
...
// This operation has its api-version elevated to the client as normal
@route("/resources")
op listResources(@query("api-version") apiVersion: string): void;

// This operation has an api-version query param that should NOT be treated
// as the API version. It stays as a method parameter.
@route("/snapshot")
op getSnapshot(
  @apiVersion(false)
  @query("api-version")
...
class ServiceClient:
    ...
    def list_resources(self, **kwargs: Any) -> None: ...

    def get_snapshot(self, *, api_version: str, **kwargs: Any) -> None: ...
```
...
public partial class ServiceClient
{
    ...
    public virtual Response GetResources(CancellationToken cancellationToken = default);
    public virtual Response GetSnapshot(string apiVersion, CancellationToken cancellationToken = default);
}
```
...
export class ServiceClient {
  ...
  listResources(options?: ListResourcesOptionalParams): Promise<void>;
  getSnapshot(apiVersion: string, options?: GetSnapshotOptionalParams): Promise<void>;
}
```
...
public final class ServiceClientClient {
    ...
    public void getSnapshot(String apiVersion);
}
```

```go
func NewServiceClientWithNoCredential(endpoint string, options *ServiceClientOptions) (*ServiceClient, error)

func (client *ServiceClient) ListResources(ctx context.Context, options *ServiceClientListResourcesOptions) (ServiceClientListResourcesResponse, error)

func (client *ServiceClient) GetSnapshot(ctx context.Context, apiVersion string, options *ServiceClientGetSnapshotOptions) (ServiceClientGetSnapshotResponse, error)
```

</ClientTabs>

In the generated client, `listResources` will use the client-level API version parameter (defaulting to `"2024-04-01"`), while `getSnapshot` will require `apiVersion` to be passed explicitly as a method parameter.

## Extending the Client API Version Enum (`@clientApiVersions`)

When generating a client, you may want the API version enum to include versions beyond those defined by the service's `@versioned` decorator. This is useful when you want to generate a complete API version enum without requiring the entire specification to be annotated with versioning decorators.

Use `@clientApiVersions` to specify the full set of API versions the client should support:
...
```python
# The generated client accepts api_version as a keyword argument.
# The API version defaults to the latest version "v5".
client = ContosoClient(endpoint="...", api_version="v1")
```

```csharp
// The ServiceVersion enum includes all five versions.
public enum ServiceVersion
{
    V1 = 1,
    V2 = 2,
...
```typescript
export enum KnownClientApiVersions {
  V1 = "v1",
  V2 = "v2",
  V3 = "v3",
  V4 = "v4",
...
In this example, the generated client will expose API versions `v1` through `v5`, even though the service spec only defines `v4` and `v5`. This is particularly useful for legacy services where adding `@added`/`@removed` annotations to every type and operation is not practical.
