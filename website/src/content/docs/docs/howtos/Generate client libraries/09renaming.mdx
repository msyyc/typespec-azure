...
In this example, the scoped renames on `ScopedModel.name` use an underscore-prefixed name per language to verify that each emitter preserves the name as-is, without stripping or re-casing it. The TypeScript emitter preserves the exact name as `_myName`; support varies by language emitter.
