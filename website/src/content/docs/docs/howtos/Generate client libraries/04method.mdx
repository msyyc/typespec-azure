...
TCGC enables both protocol and convenience methods by default. The Java and C# emitters expose these as separate method layers. Other emitters may represent an operation with a single API surface instead.

> **NOTE:** Python and TypeScript don't have a separation of convenience and protocol methods.
...
Java and C# emitters support global generation parameters:
...
To set global emitter parameters, read the documentation of [emitter configuration](https://microsoft.github.io/typespec/introduction/configuration#configuration-file).

Setting either option for any other language emitter produces an `unnecessary-emitter-option` warning because the option has no effect.
...
Use the augment operator in `client.tsp`. This example disables convenience methods for Java and C# while leaving the other language emitters unchanged.
...
@route("/pets/{name}")
@get
op getPet(@path name: string): Pet;
```
...
@@convenientAPI(PetStore.getPet, false, "java, csharp");
```

```python
class PetStoreClient:
    def __init__(self, endpoint: str, **kwargs: Any) -> None: ...
    def get_pet(self, name: str, **kwargs: Any) -> Pet: ...
```
...
namespace PetStore
{
  ...
  {
    public virtual Response GetPet(string name, RequestContext context = null) { ... }
    public virtual async Task<Response> GetPetAsync(string name, RequestContext context = null) { ... }
  }
}
```
...
export class PetStoreClient {
  ...
  getPet(name: string, options: GetPetOptionalParams = { requestOptions: {} }): Promise<Pet>;
}
```

```java
public final class PetStoreClient {
    public Response<BinaryData> getPetWithResponse(String name, RequestOptions requestOptions) { ... }
}

public final class PetStoreAsyncClient {
    public Mono<Response<BinaryData>> getPetWithResponse(String name, RequestOptions requestOptions) { ... }
}
```
...
func NewPetStoreClientWithNoCredential(endpoint string, options *PetStoreClientOptions) (*PetStoreClient, error)

func (client *PetStoreClient) GetPet(ctx context.Context, name string, options *PetStoreClientGetPetOptions) (PetStoreClientGetPetResponse, error)
```
...
#### `replaceResponseWithVoid` and `replaceResponseWithBytes`

Replace an operation's client method response type while preserving the HTTP response metadata used for the wire protocol. Use `replaceResponseWithVoid` when the response should not be exposed to SDK callers, or `replaceResponseWithBytes` when callers should receive the raw response body.

```typespec
extern fn replaceResponseWithVoid(operation: Reflection.Operation): Reflection.Operation;
extern fn replaceResponseWithBytes(operation: Reflection.Operation): Reflection.Operation;
```

Apply the transformation with `@@override`:

```typespec
@service
namespace MyService;

op deleteWidget(): string;
op downloadWidget(): string;
```
...
#suppress "experimental-feature" "using replaceResponseWithVoid"
alias DeleteResponse = replaceResponseWithVoid(MyService.deleteWidget);
@@override(MyService.deleteWidget, DeleteResponse);

#suppress "experimental-feature" "using replaceResponseWithBytes"
alias DownloadResponse = replaceResponseWithBytes(MyService.downloadWidget);
@@override(MyService.downloadWidget, DownloadResponse);
```
